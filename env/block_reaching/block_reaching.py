...
from env.block_reaching.utils import utils_pybullet
from env.block_reaching.utils import xarm_sim_robot
from env.block_reaching.utils.pose3d import Pose3d
from env.block_reaching.utils.utils_pybullet import ObjState
from env.block_reaching.utils.utils_pybullet import XarmState
import numpy as np
...
INITIAL_JOINT_POSITIONS = np.array([-0.642566870821590, 0.709391125787550, -1.084525949913924, 0.000147836863717, 0.375143859118023, -3.784298052534032 ])
...
class BlockPush(gym.Env):
    ...
    def __init__(
        ...
        # 修改相机
        #self._previous_effector_pos = None
    ...
    def reset(self, reset_poses=True):
        ...
        # 修改相机
        #self._previous_effector_pos = self._robot.forward_kinematics().translation
    ...
    def calc_camera_params(self, image_size):
        ...
        # 相机位置
        front_position = np.array([0.4, -0.2, 0.07])
        effector_pos = self._robot.forward_kinematics().translation
        camera_offset = np.array([0.0, 0.01, -0.02])
        front_position = effector_pos + camera_offset

        # 相机看向

        if self._previous_effector_pos is not None:
            effector_vel = effector_pos[:2] - self._previous_effector_pos[:2]
            speed = np.linalg.norm(effector_vel)
            #if speed > 1e-6:
            direction = effector_vel / speed
        else:
            direction = np.array([0.4, 0.2])

        # lookat 点：沿运动方向看出去一点
        lookat = np.array([front_position[0], front_position[1], 0.01])+np.array([direction[0], direction[1], 0.0])
        #lookat = np.array([0.5, 0.2, 0.05])

        #print("lookat:", lookat)

        # 设置相机旋转（这里是 roll, pitch, yaw）
        # 我们需要把up向量设置正确
        # PyBullet默认相机的“正上”向量是Y轴负方向
        front_rotation = np.array([0.0, 0.0, 0.0])  # 不转动

        # 生成四元数
        front_rotation_quat = self._pybullet_client.getQuaternionFromEuler(front_rotation)
        rotation = self._pybullet_client.getMatrixFromQuaternion(front_rotation_quat)
        rotm = np.float32(rotation).reshape(3, 3)

        # 计算lookdir和updir
        lookdir = (rotm @ np.array([0, 0, 1])).reshape(-1)  # 正Z方向
        updir = (rotm @ np.array([0, 0, 1])).reshape(-1)  # 负Y方向

        updir = np.array([0, 0, 1])

        # 这里我们其实只要确保地板在下方，保持默认方向就行

        znear, zfar = 0.01, 10.0

        viewm = self._pybullet_client.computeViewMatrix(
            front_position, lookat, updir
        )
...
for env_id in [
    "BlockInsert-v0",
    "BlockPush-v0",
    "BlockPushNormalized-v0",
    "BlockPushRgbNormalized-v0",
    "BlockReach-v0",
    "BlockReachNormalized-v0",
    "BlockReachRgbNormalized-v0",
    "SharedBlockInsert-v0",
    "SharedBlockPush-v0",
    "SharedBlockPushNormalized-v0",
    "SharedBlockReach-v0"
]:

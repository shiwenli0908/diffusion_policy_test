...
from env.block_reaching.block_reaching import *
...
class BlockPushMultimodal(BlockPush):
    ...
    def __init__(
        ...
        task=BlockTaskVariant.REACH,
        image_size=None,
        shared_memory=False,
        seed=None,
        goal_dist_tolerance=0.05,
        abs_action=False,
        custom_block_positions: Optional[List[List[float]]] = None
    ):
        ...
        self.custom_block_positions = custom_block_positions
    ...
    def _setup_pybullet_scene(self):
        ...
        self._target_ids = [
            ...
            for i in [ZONE_URDF_PATH, ZONE2_URDF_PATH]
        ]
        self._block_ids = []
        for i in [BLOCK_URDF_PATH, BLOCK2_URDF_PATH]:
    ...
    def _reset_block_poses(self, workspace_center_x):
        ...
        def _reset_block_pose(idx, add=0.0, avoid=None):
            def _get_random_translation():
                ...
                '''
                if idx==0:
                    #block_x, block_y = 0.3, 0.21
                    pos = self.custom_block_positions[idx]
                    block_x, block_y = pos[0], pos[1]
                elif idx==1:
                    #block_x, block_y = 0.5, 0.21
                    pos = self.custom_block_positions[idx]
                    block_x, block_y = pos[0], pos[1]
        ...
        def _reset_target_pose(idx, add=0.0, avoid=None):
            def _get_random_translation():
                ...
                if idx==0:
                    ...
                    target_x, target_y = 0.5, 0.2

                target_translation = np.array([target_x, target_y, 0.00])
    ...
    def reset(self, reset_poses=True):
        ...
        if reset_poses:
            ...
            translation = np.array([0.4, -0.3, EFFECTOR_HEIGHT])
    ...
    def _get_reward(self, state):
        effector_pos = state["effector_translation"]
        block0_pos = state["block_translation"]
        block1_pos = state["block2_translation"]

        '''
        print("Effector position:", effector_pos)
        print("Block 0 position:", block0_pos)
        print("Block 1 position:", block1_pos)
        '''

        dist0 = np.linalg.norm(effector_pos - block0_pos)
        dist1 = np.linalg.norm(effector_pos - block1_pos)

        if dist0 < self.goal_dist_tolerance or dist1 < self.goal_dist_tolerance:
            return 1.0  # 满足靠近条件，给予满奖励
        return 0.0
...
if "BlockPushMultimodal-v0" in registry.env_specs:
    del registry.env_specs["BlockPushMultimodal-v0"]

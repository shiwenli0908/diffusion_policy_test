...
obs_dim = vision_feature_dim  + lowdim_obs_dim + target_dim
action_dim = 2

#global_cond_dim = (vision_feature_dim + lowdim_obs_dim + target_dim) * obs_horizon + potential_feature_dim + obs_horizon
global_cond_dim = (vision_feature_dim + lowdim_obs_dim + target_dim) * obs_horizon + 2*obs_horizon
...
if load_pretrained:
  ckpt_path = "/home/wenli/checkpoints/blockreach_vision_cfg_vision_400.ckpt"
...
'''# ==== 加载 potential map ====
potential_path = os.path.join(scene_dir, f"potential_goal{target_idx}.npy")
potential_map = np.load(potential_path)  # (128,128)
potential_tensor = torch.from_numpy(potential_map).unsqueeze(0).unsqueeze(0).float().to(device)  # (1,1,H,W)
potential_tensor = F.interpolate(potential_tensor, size=(96, 96), mode='bilinear', align_corners=False)'''
...
obs_deque = collections.deque(
    [{"image": frame_proc, "obs": obs_vec.copy()}] * obs_horizon,
    maxlen=obs_horizon
)
...
with torch.no_grad():
    while not done and step_idx < max_steps:
        B = 1
        images = np.stack([x["image"] for x in obs_deque])  # (obs_h,3,96,96)
        nimages = torch.from_numpy(images).float().to(device)
        ...
        image_features = nets["vision_encoder"](nimages)  # (obs_h, 512)
        obs_feat = torch.cat([image_features, nagent], dim=-1)  # (obs_h, 514)
        ...
        for act in act_seq:
            ...
            #obs_vec = retransform(obs_vec)  # 将 effector_pos 转换回原始空间
            obs_deque.append({"image": frame_proc, "obs": obs_vec})
            imgs.append(frame)
...
video_path = f"videos/infer_scene{scene_idx}_target{target_idx}.mp4"

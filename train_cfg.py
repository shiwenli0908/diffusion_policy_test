    ...
    for epoch_idx in tglobal:
        ...
        with tqdm(dataloader, desc='Batch', leave=False) as tepoch:
            for nbatch in tepoch:
                ...
                # (4) obs embedding
                obs_features = torch.cat([image_features, nagent_pos, target], dim=-1)  # (B, obs_horizon, D)
...
torch.save(ema_nets.state_dict(), "blockreach_vision_cfg.ckpt")
print("EMA model saved to blockreach_ema.ckpt")

# 保存目录
save_dir = "training_plots"
os.makedirs(save_dir, exist_ok=True)
save_path = os.path.join(save_dir, "loss_curve_epoch_cfg.png")

plt.figure(figsize=(8, 5))
plt.plot(all_epoch_losses, label='Train Loss')
plt.xlabel('Epoch')
plt.ylabel('MSE Loss (log scale)')
plt.title('Training Loss Curve for Vision-Traget-Conditioned Diffusion Policy')
plt.yscale('log')
